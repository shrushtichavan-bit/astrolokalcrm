...
export async function listUsers() {
  ...
  const { data, error } = await supabaseAdmin.from("users").select("id, name, email, role, created_at").order("name");
...
export async function addUser(input: { name: string; email: string; password: string; role: string }) {
  ...
  const { error } = await supabaseAdmin.from("users").insert({ name: data.name.trim(), email, password_hash, role: data.role });
