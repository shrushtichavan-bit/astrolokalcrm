...
export async function upsertSourcePriority(input: { source_name: string; priority_score: number; is_active: boolean }) {
  const data = z
    .object({
  ...
  const { error } = await supabaseAdmin.from("source_priority_config").upsert(
    { source_name: data.source_name.trim(), priority_score: data.priority_score, is_active: data.is_active, updated_at: new Date().toISOString() },
