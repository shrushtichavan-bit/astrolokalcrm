...
import { getStageAssignmentCounts } from "@/lib/assignments.functions";
...
function PeoplePage() {
  ...
  const assignedFn = useServerFn(getStageAssignmentCounts);
  const assignedQ = useQuery({
    queryKey: ["admin-stage-assignment-counts"],
    queryFn: () => assignedFn(),
  });
...
function futureAssignedFor(assignedRows: AssignedRow[], stage: string, email: string): number {
  return assignedRows.find((r) => r.stage === stage && r.assigned_email === email)?.count ?? 0;
}

function CallersCard({
  from,
  to,
  assignedRows,
}: {
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2 text-right">Future Assigned</th>
            </tr>
          </thead>
          <tbody>
            {(q.data?.rows ?? []).map((r) => (
              <tr key={r.email} className="border-t">
                ...
                <td className="px-3 py-2 text-right tabular-nums">
                  {futureAssignedFor(assignedRows, "calling", r.email)}
                </td>
              </tr>
            ))}
          ...
          <tbody>
            {(q.data?.rows ?? []).map((r) => (
              <tr key={r.email} className="border-t">
                ...
                <td className="px-3 py-2 text-right tabular-nums">
                  {futureAssignedFor(assignedRows, `round_${round}`, r.email)}
                </td>
              </tr>
            ))}
            {!q.isLoading && (q.data?.rows ?? []).length === 0 && (
          ...
          <tbody>
            {(q.data?.rows ?? []).map((r) => (
              <tr key={r.email} className="border-t">
                ...
                <td className="px-3 py-2 text-right tabular-nums">
                  {futureAssignedFor(assignedRows, "expert_creation", r.email)}
                </td>
              </tr>
            ))}
            {!q.isLoading && (q.data?.rows ?? []).length === 0 && (
