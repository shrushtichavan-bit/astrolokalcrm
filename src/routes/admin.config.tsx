...
import { listUsers } from "@/lib/team.functions";
...
function RoundConfigTab() {
  ...
  const getFn = useServerFn(getRoundConfig);
  const saveFn = useServerFn(upsertRoundConfig);
  const q = useQuery({ queryKey: ["admin-round-config"], queryFn: () => getFn() });

  const [numRounds, setNumRounds] = useState(2);
  const [requiredForVerdict, setRequiredForVerdict] = useState(2);
    ...
    try {
      await saveFn({
        data: {
          ...
          passing_marks: Array.from({ length: numRounds }, (_, i) => ({
            round_number: i + 1,
            passing_marks: marks[i + 1] ?? 0,
          })),
        },
      });
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input
              ...
              min={1}
              max={numRounds}
              value={requiredForVerdict}
              onChange={(e) => setRequiredForVerdict(parseInt(e.target.value, 10) || 1)}
            />
          </div>
        </div>
            ...
            {Array.from({ length: numRounds }, (_, i) => i + 1).map((n) => (
              <div key={n}>
                ...
                <Input
                  ...
                  min={0}
                  value={marks[n] ?? 0}
                  onChange={(e) => setMarks({ ...marks, [n]: parseInt(e.target.value, 10) || 0 })}
                />
              </div>
            ))}
          </div>
...
function QuestionsTab() {
  ...
  const qc = useQueryClient();
  const getFn = useServerFn(getQuestions);
  const saveFn = useServerFn(upsertQuestions);
  const q = useQuery({
    queryKey: ["admin-questions", round],
    queryFn: () => getFn({ data: { round_number: round } }),
  });
  ...
  function addRow() {
    setRows([
      ...rows,
      { question_id: `q${rows.length + 1}`, question_text: "", display_order: rows.length },
    ]);
  }
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      await saveFn({ data: { round_number: round, questions: rows } });
    ...
    <Card>
      <CardContent className="space-y-4 p-4">
        ...
        <div className="space-y-3">
          {rows.map((r, i) => (
            <div key={i} className="flex gap-2 rounded-md border p-3">
              <div className="w-28 shrink-0">
                <Label className="text-xs text-muted-foreground">Question ID</Label>
                <Input
                  value={r.question_id}
                  onChange={(e) => updateRow(i, { question_id: e.target.value })}
                />
              </div>
              ...
              <Button size="sm" variant="outline" className="self-end" onClick={() => removeRow(i)}>
                Remove
              </Button>
            </div>
          ))}
...
function StagePoolsTab() {
  const [stage, setStage] = useState<string>("calling");
  const qc = useQueryClient();
  const getFn = useServerFn(getStagePools);
  const usersFn = useServerFn(listUsers);
  const saveFn = useServerFn(upsertStagePools);
  const poolsQ = useQuery({ queryKey: ["admin-stage-pools"], queryFn: () => getFn() });
  const usersQ = useQuery({ queryKey: ["admin-team"], queryFn: () => usersFn() });

  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    const emails = (poolsQ.data?.pools ?? [])
      .filter((p) => p.stage === stage)
      .map((p) => p.eligible_email);
  ...
  async function save() {
    setBusy(true);
    try {
      await saveFn({ data: { stage, eligible_emails: Array.from(selected) } });
        ...
        <div className="space-y-2">
          ...
          {!usersQ.isLoading && (usersQ.data?.users ?? []).length === 0 && (
            <p className="text-sm text-muted-foreground">
              No team members yet — add some in Team first.
            </p>
          )}
        </div>
        <Button onClick={save} disabled={busy}>
          {busy ? "Saving…" : `Save ${STAGE_LABELS[stage]} Pool`}
        </Button>
      </CardContent>
    </Card>
  );
